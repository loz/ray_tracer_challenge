...
5.times do |x|
  5.times do |z|
    5.times do |y|
      wx = (Random.rand(10) / 10.0) - 0.5
      wy = (Random.rand(10) / 10.0) - 0.5
      wz = (Random.rand(10) / 10.0) - 0.5
      ball = PhysicsObject.new point(-1.5 + (x*0.75) + wx, y * 0.75 + wy, -1.5 + (z*0.75) + wz), vector(0.0, 0.0, 0.0)
      ball.shape.material = water_mat
      pworld << ball
      if y > 0
        g = CSG.new(:union, g, ball.shape)
      else 
        g = ball.shape
      end
    end
...
if anim
  ...
  30.times { pworld.tick(0.1) }
  single(camera, world, "water2")
end
