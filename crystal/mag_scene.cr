...
def lens(depth=0.2)
  front = sphere()
  front.material.color = black
  front.material.transparency = 1.0
  front.material.refractive_index = 1.5
  front.transform = translation(0.0, 0.0, -1.0 + depth)
  back = sphere()
  back.transform = translation(0.0, 0.0, 1.0 - depth)
  back.material.color = black
  back.material.transparency = 1.0
  back.material.refractive_index = 1.5
  c = CSG.new(:intersection, back, front)
  c
end
...
mag = lens()
mag.transform = translation(-2.5, 3.0, -4.0)
world.objects << mag
...
frames.times do |f|
  seq = sprintf("%03d", f)
  p seq
  mag.transform = translation(-2.5 + (f * mov), 3.0, -4.0) *
                  scaling(2.0, 2.0, 2.0) *
		  rotation_x(Math::PI/16.0)
